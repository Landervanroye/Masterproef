...
#deze keer: nauwkeurigheid 1/sqrt(lr)!
...
while z < length(plist)
    ...
    global gempos;
    global steps;
    file = matopen(string("exp17res/poskeep", seed, ".mat"), "w")
    write(file, "poskeep", db)
    close(file)

    info = [z,j,lrlist[z],stationary, steps];
    file = matopen(string("exp17res/info", seed, ".mat"), "w")
    ...
    file = matopen(string("exp17res/grad", seed, ".mat"), "w")
    ...
    if stationary && j == steps
        #db = gempos;
        file = matopen(string("exp17res/gempos", seed, ".mat"), "w")
