...
function simulate_adjoint_MC(T, U,W,X,D,deltax,deltaxp,nu)
    ...
    for s = 1:nbp
        ...
        lambda[NT, s]= 0.5*deltat*deltax*U[NT,bNT];
...
function simulate_adjoint_MC_rng(T,U,D,samples_beg, weights_beg, randgen,deltax,deltaxp,nu, problem, debdiscr, MC_discr)
    ...
    for s = 1:nbp
        ...
        lambda= 0.5*deltat*deltax*U[NT,bNT];
...
r= 0.5*deltaxp*nu*dotp(d,d)+0.5*(deltat*0.5*(dotp(Uout[1,:],Uout[1,:])deltax)+deltat*0.5*(dotp(Uout[length(T),:],Uout[length(T),:])deltax));
for i2 = 2:(length(T)-1)
