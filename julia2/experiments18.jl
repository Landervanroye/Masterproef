...
while z < length(plist)
    ...
    global gempos;
    global steps;
    file = matopen(string("exp18res/poskeep", seed, ".mat"), "w")
    write(file, "poskeep", db)
    close(file)

    info = [z,j,lrlist[z],stationary, steps];
    file = matopen(string("exp18res/info", seed, ".mat"), "w")
    ...
    file = matopen(string("exp18res/grad", seed, ".mat"), "w")
    ...
    if stationary && j == steps
        db = gempos;
        file = matopen(string("exp18res/gempos", seed, ".mat"), "w")
