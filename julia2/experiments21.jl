...
while z < length(plist)
    ...
    global gempos;
    global steps;
    file = matopen(string("exp21res/poskeep", seed, ".mat"), "w")
    write(file, "poskeep", db)
    close(file)

    info = [z,j,lrlist[z],stationary, steps];
    file = matopen(string("exp21res/info", seed, ".mat"), "w")
    ...
    file = matopen(string("exp21res/grad", seed, ".mat"), "w")
    ...
    if j == steps
        #db = gempos;
        file = matopen(string("exp21res/gempos", seed, ".mat"), "w")
